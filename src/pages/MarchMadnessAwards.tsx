...
function MarchMadnessAwards() {
  const mostPoints = accolades.find(
    (accolade) => accolade.id === "successfulPeople",
  ) as AccoladeInfo;
  const mostWins = accolades.find(
    (accolade) => accolade.id === "accuratePeople",
  ) as AccoladeInfo;
  const indecisivePeople = accolades.find(
    (accolade) => accolade.id === "indecisivePeople",
  ) as AccoladeInfo;
  const ambitiousPeople = accolades.find(
    (accolade) => accolade.id === "ambitiousPeople",
  ) as AccoladeInfo;
  const bestBracketTitles = accolades.find(
    (accolade) => accolade.id === "bestBracketTitles",
  ) as AccoladeInfo;
  const leastWins = accolades.find(
    (accolade) => accolade.id === "inaccuratePeople",
  ) as AccoladeInfo;
  const leastPoints = accolades.find(
    (accolade) => accolade.id === "unsuccessfulPeople",
  ) as AccoladeInfo;
    ...
    <section className="section">
      <div className="container">
        ...
        <h2 className="subtitle has-text-centered">
          A collection of awards, highlights, and lowlights from the{" "}
          {CURRENT_YEAR} March Madness Tournament
        </h2>
        <br />
        <br />
        ...
        <div className="columns">
                ...
                {ambitiousPeople.data.map((info, index) => {
                  return (
                    <tr key={`${info.userId}-ambitiousPeople`}>
                      ...
                      <td>{info.points}</td>
                      <td>
                        {((info.points / info.startingMaxPoints) * 100).toFixed(
                          2,
                        )}
                        %
                      </td>
          ...
          <div className="column">
            <h2 className="title has-text-centered">
              {indecisivePeople.title}
            </h2>
            <h3 className="subtitle has-text-centered">
              {indecisivePeople.description}
            </h3>
            <table className="table is-striped is-hoverable mx-auto">
              <thead>
                <tr>
        ...
        <div className="columns">
          <div className="column">
            <h2 className="title has-text-centered">
              {bestBracketTitles.title}
            </h2>
            <h3 className="subtitle has-text-centered">
              {bestBracketTitles.description}
            </h3>
            <table className="table is-striped is-hoverable mx-auto">
              <thead>
                <tr>
