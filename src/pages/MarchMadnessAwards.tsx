    ...
    return (
        <section className='section'>
            <div className='container'>
                ...
                <h2 className='subtitle has-text-centered'>A collection of awards, highlights, and lowlights from the {CURRENT_YEAR} March Madness Tournament</h2>
                <br /><br />
                            ...
                            <tbody>
                                {
                                    ambitiousPeople.data.map((info, index) => {
                                        return <tr key={`${info.userId}-ambitiousPeople`}>
                                            <td>{index + 1}</td>
                                            <td>{`${info.firstName} ${info.lastName}`}</td>
                                            <td><b>{info.startingMaxPoints}</b></td>
                                            <td>{info.points}</td>
                            ...
                            <tbody>
                                {
                                    indecisivePeople.data.map((info, index) => {
                                        return <tr key={`${info.userId}-indecisivePeople`}>
                                            <td>{index + 1}</td>
                                            <td>{`${info.firstName} ${info.lastName}`}</td>
                                            <td><b>{info.timesUpdated}</b></td>
                                        </tr>
                                    })
                                }
                            </tbody>
                            ...
                            <tbody>
                                {
                                    bestBracketTitles.data.map((info, index) => {
                                        return <tr key={`${info.userId}-bestBracketTitles`}>
                                            <td>{index + 1}</td>
                                            <td>{`${info.firstName} ${info.lastName}`}</td>
                                            <td><b>{info.bracketTitle}</b></td>
                                        </tr>
                                    })
                                }
                            </tbody>
                            ...
                            <tbody>
                                {
                                    leastWins.data.map((info, index) => {
                                        return <tr key={`${info.userId}-leastWins`}>
                                            <td>{index + 1}</td>
                                            <td>{`${info.firstName} ${info.lastName}`}</td>
                                            <td>{info.points}</td>
                                            <td><b>{info.numCorrect}</b></td>
                            ...
                            <tbody>
                                {
                                    leastPoints.data.map((info, index) => {
                                        return <tr key={`${info.userId}-leastPoints`}>
                                            <td>{index + 1}</td>
                                            <td>{`${info.firstName} ${info.lastName}`}</td>
                                            <td><b>{info.points}</b></td>
                                            <td>{info.numCorrect}</td>
