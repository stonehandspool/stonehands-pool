...
function PersonalStats() {
      ...
      <section className="section">
        <div className="container">
          <h3 className="title is-3 has-text-centered">
            Sorry, there is no data to show yet
          </h3>
        </div>
      </section>
    );
  ...
  const userInfo: UserInfo = players.find(
  ...
  Object.keys(weeklyPicks.weeklyPicks).map((key, index) => {
    ...
    if (week && week.length > 0 && index < weekToShow) {
      const playerPicksThisWeek = week.find(
        (submission) => submission.submission_data.username === username,
      );
  ...
  if (userInfo) {
    ...
    survivorColor = userInfo.aliveInSurvivor
      ? "has-text-success"
      : "has-text-danger";
    marginColor =
      userInfo.marginTotal > 0 ? "has-text-success" : "has-text-danger";
    ...
    if (weekToShow < CURRENT_WEEK) {
      unusedSurvivorPicks.push(
        userInfo.survivorPicks[userInfo.survivorPicks.length - 1],
      );
    ...
    if (weekToShow < CURRENT_WEEK) {
      unusedMarginPicks.push(
        userInfo.marginPicks[userInfo.marginPicks.length - 1].team,
      );
    ...
    bestMarginWeek =
      userInfo.marginPicks.findIndex((pick) => pick.team === team) + 1;

    worstMargin = userInfo.marginPicks.reduce((prev, current) => {
      return prev.margin > current.margin && current.margin !== null
        ? current
        : prev;
    });
    const { team: worst, margin: worstVal } = worstMargin;
    worstMarginWeek =
      userInfo.marginPicks.findIndex((pick) => pick.team === worst) + 1;
  }
      ...
      <div className="container">
        <div className="mb-5">
          <h1 className="title has-text-centered">
            The stat page for {`${userInfo.firstName} ${userInfo.lastName}`}
          </h1>
          <h2 className="subtitle has-text-centered">
            In-depth state for the {CURRENT_YEAR} season
          </h2>
        </div>
        <div className="mb-6">
          ...
          <div className="box level">
            ...
            <div className="level-item has-text-centered">
              <div>
                ...
                <p className={`title ${survivorColor}`}>
                  {userInfo.aliveInSurvivor ? "Alive" : "Eliminated"}
                </p>
              </div>
            </div>
            <div className="level-item has-text-centered">
              <div>
                ...
                <p
                  className={`title ${marginColor}`}
                >{`${marginOperator}${userInfo.marginTotal}`}</p>
              </div>
            </div>
            <div className="level-item has-text-centered">
              <div>
        ...
        <div className="container">
          <div className="tabs is-centered is-boxed">
            <ul>
              ...
              <li
                className={activeChoice === Pools.Survivor ? "is-active" : ""}
              >
              ...
              <li
                className={activeChoice === Pools.HighFive ? "is-active" : ""}
              >
          ...
          <div className="container">
            {activeChoice === Pools.Confidence && (
              <UserConfidenceReport
                userPicks={userPicks}
                teamsByPicks={teamsByPicks}
            ...
            {activeChoice === Pools.HighFive && (
              <UserHighFiveReport userInfo={userInfo} weekToShow={weekToShow} />
            )}
          </div>
        </div>
      </div>
