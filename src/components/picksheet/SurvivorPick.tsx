...
function SurvivorPick(props: any) {
  ...
  return (
    <div className="container pb-6">
      ...
      {userInfo.aliveInSurvivor && (
        <>
          <h4 className="subtitle">
            Pick one team each week that you are certain will win. You can only
            pick each team <b>once</b>. If you are right you will continue on to
            the next week, if you are wrong you are eliminated from the survivor
            pool, so be careful!
          </h4>
          {priorPickGameCompleted && (
            <h6 className="subtitle is-6 has-text-danger">
              Your choice from this week has already played their game. you can
              no longer change your pick
            </h6>
          )}
      ...
      {!userInfo.aliveInSurvivor && (
        <h4 className="subtitle">
          Sorry, you are no longer in the survivor pool, maybe next year!
        </h4>
      )}
    </div>
  );
