...
function PickSheetForm(props: PicksheetFormProps) {
  ...
  const handleBracketTitleInput = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (allPicksMade && tiebreaker !== '') {
      setAllPicksFilled(true);
    } else if ((!allPicksMade && allPicksFilled) || (tiebreaker === '' && allPicksFilled)) {
      setAllPicksFilled(false);
    }
  };
            ...
            {Array.from(Array(16).keys()).map(index => {
              return (
                <MatchupCard
                  key={`round-of-64-${index}`}
                  customClass="col-2"
                  matchupInfo={userPicks[32 + index]}
                  onClick={handleClick}
                />
              );
            })}
            ...
            {Array.from(Array(8).keys()).map(index => {
              return (
                <MatchupCard
                  key={`round-of-64-${index}`}
                  customClass="col-3"
                  matchupInfo={userPicks[48 + index]}
                  onClick={handleClick}
                />
              );
            })}
            ...
            {Array.from(Array(4).keys()).map(index => {
              return (
                <MatchupCard
                  key={`round-of-64-${index}`}
                  customClass="col-4"
                  matchupInfo={userPicks[56 + index]}
                  onClick={handleClick}
                />
              );
            })}
            ...
            {Array.from(Array(2).keys()).map(index => {
              return (
                <MatchupCard
                  key={`round-of-64-${index}`}
                  customClass="col-5"
                  matchupInfo={userPicks[60 + index]}
                  onClick={handleClick}
                />
              );
            })}
