...
function ConfidenceYearlyConsensusTable() {
    ...
    weekPicks.forEach((pickInfo) => {
      ...
      for (let j = 0; j < numGames; j++) {
        ...
        const userConfidence = parseInt(
          picks[`matchup-${j}-confidence` as keyof typeof picks] as string,
          10,
        );
        const homeTeamConsensusInfo = yearlyConsensusArr.find(
          (info) => info.teamName === homeTeam,
        );
        const awayTeamConsensusInfo = yearlyConsensusArr.find(
          (info) => info.teamName === awayTeam,
        );

        if (
          homeTeamConsensusInfo !== undefined &&
          awayTeamConsensusInfo !== undefined
        ) {
  ...
  yearlyConsensusArr.forEach((consensusInfo) => {
    if (consensusInfo.numTimesPicked === 0) {
      ...
      const totalPicks =
        consensusInfo.numTimesPicked + consensusInfo.numTimesOppPicked;
      consensusInfo.percent = `${((consensusInfo.numTimesPicked / totalPicks) * 100).toFixed(1)}%`;
      consensusInfo.pickDiff =
        consensusInfo.numTimesPicked - consensusInfo.numTimesOppPicked;
      consensusInfo.pointDiff =
        consensusInfo.pointsFor - consensusInfo.pointsAgainst;
    }
  });
    ...
    <table className="table is-hoverable has-text-centered">
      <thead>
        <tr>
          ...
          <th className="has-text-centered">
            <abbr title="Number of Times Opponent was Picked"></abbr># Times{" "}
            <br /> Opponent Picked
          </th>
          ...
          <th className="is-vcentered">
            <abbr title="Cumulative Points from all times picked"></abbr>Points
            For
          </th>
          <th className="is-vcentered">
          ...
          return (
            <tr key={`row-${index}`}>
              {Object.keys(info).map((key, index) => {
                if (index === 0) {
                  const Logo =
                    TeamLogos[info.teamName as keyof typeof TeamLogos];
                  return (
                    <td
                      key={`${info.teamName}-yearly`}
