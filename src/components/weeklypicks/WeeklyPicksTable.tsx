...
function WeeklyPicksTable() {
      ...
      <section className="section">
        <div className="container">
          <h3 className="title is-3 has-text-centered">
            Sorry, there are no picks to show yet
          </h3>
        </div>
      </section>
    );
  ...
  weeklyPicks.sort((row1, row2) => {
    ...
    return (
      lastName1.localeCompare(lastName2) || firstName1.localeCompare(firstName2)
    );
  });
      ...
      <tbody>
        ...
        <tr>
          ...
          {matchupKeys.map((key, index) => {
            const { away_team, away_score } =
              currentWeek[key as keyof typeof currentWeek];
            return (
              <td key={`away-${index}`}>
                {away_team}
                <br />
        ...
        {weeklyPicks.map((pickInfo, index) => {
          ...
          const playerInfo = players.find(
            (player) => player.id === pickInfo.user_id,
          );
          return (
            <tr key={`picks-${index}`}>
              ...
              {matchupKeys.map((key, index) => {
                ...
                const confidence =
                  picks[`matchup-${index}-confidence` as keyof typeof picks];
                ...
                if ((!showAllPicks && evaluated) || showAllPicks) {
                  ...
                  return (
                    <td
                      ...
                      className={
                        correct
                          ? "weekly-picks-table-correct-choice"
                          : "weekly-picks-table-incorrect-choice"
                      }
                    >
              ...
              <td className="is-vcentered">
                {playerInfo?.currentWeekTiebreaker}
              </td>
