...
function SurvivorTable() {
    ...
    playerPicks.sort((row1, row2) => {
        ...
        return row2.survivorPicks.length - row1.survivorPicks.length || Number(row2.aliveInSurvivor) - Number(row1.aliveInSurvivor) || lastName1.localeCompare(lastName2) || firstName1.localeCompare(firstName2);
    });
                ...
                <table className='table is-bordered is-striped is-hoverable mx-auto'>
                    ...
                    <tbody>
                            {playerPicks.map((row, index) => {
                                return <tr key={`${index}`}>
                                    <td key={`${row.name}-row-${index}`} className='has-text-centered'>{row.name}</td>
                                    {
