...
const weeklyResults =
  seasonResults.weeks[
    `week_${CURRENT_WEEK}` as keyof typeof seasonResults.weeks
  ];
const getGameCompleted = (teamName: string) => {
  ...
  Object.keys(weeklyResults).map((key) => {
    ...
    if (
      matchupInfo.home_team === teamName ||
      matchupInfo.away_team === teamName
    ) {
              ...
              return (
                <tr key={`${index}`}>
                  ...
                  {weeksArr.map((week, ind) => {
                    const gameCompleted = getGameCompleted(
                    ...
                    if (isCurrentWeek && !showAllPicks && !gameCompleted) {
                      ...
                      if (
                        row.marginPicks[ind].margin !== null &&
                        (row.marginPicks[ind].margin!) > 0
                      ) {
