...
if (data) {
  ...
  data.forEach(pickInfo => {
    const { submission_data: submissionData, user_id: userId, times_updated: timesUpdated, tiebreaker } = pickInfo;
    const {
      firstName,
      lastName,
      username,
  ...
  console.log(
    `Created a new file at data/${year}/marchmadness/playerPicks.json in order to keep track of the players results`
  );
}
